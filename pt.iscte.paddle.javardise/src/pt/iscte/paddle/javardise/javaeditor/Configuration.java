...
import pt.iscte.paddle.javardise.BinaryExpressionWidget;
...
import pt.iscte.paddle.javardise.UnaryExpressionWidget;
...
public class Configuration implements LanguageConfiguration {
	...
	@Override
	public Creator match(IExpression e) {
	...
	public void configure(SimpleExpressionWidget expression) {
		...
		expression.addRule((t,c) -> t.isAtEnd() && match(c, JavaConstants.BINARY_OPERATORS) != null,
				(p,t,c) -> {
					BinaryExpressionWidget w = new BinaryExpressionWidget(p, p2 -> new SimpleExpressionWidget(p2, t.getText()), match(c, JavaConstants.BINARY_OPERATORS));
					w.focusRight();
					return w;
				});
